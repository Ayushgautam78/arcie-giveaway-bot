    ...
    if (data.authenticated && data.user) {
      ...
      authContainer.innerHTML = `
        <div style="display: flex; gap: 8px;">
          <button id="loginBtn" class="btn btn-purple btn-sm" onclick="window.location.href=apiUrl('/api/auth/login')">
            Sign in Discord
          </button>
          <button class="btn btn-outline btn-sm" onclick="openModal('passLoginModal')">
            🔐 Admin Pass
          </button>
        </div>
      `;
